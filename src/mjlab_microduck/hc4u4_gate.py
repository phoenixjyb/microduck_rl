"""Apply the unchanged unified-controller numerical gate to the HC4-U4 pilot."""
...
HELD_OUT_SEEDS = (347, 349, 353)
STAGE = "HC4U4-near-state-correction-phase-BC-rollout"


def compare_hc4u4_prescreen(
    candidate: Path, near_source: Path, far_source: Path,
    *, candidate_sha256: str, seed: int,
) -> dict:
    if seed not in HELD_OUT_SEEDS:
        raise ValueError("seed is outside the predeclared HC4-U4 matrix")
    ...
    for path in (candidate, near_source, far_source):
        ...
        if report.get("terminal_outcome_protocol") != FIRST_TERMINAL_OUTCOME_PROTOCOL:
            raise ValueError("HC4-U4 requires failure-priority terminal accounting")
        ...
        if not isinstance(sensor, dict) or any(sensor.get(field) != 0.0 for field in (
            ...
            raise ValueError("HC4-U4 requires exact sensor settings")
        for case in report["cases"]:
            if case.get("terminal_outcome_protocol") != FIRST_TERMINAL_OUTCOME_PROTOCOL:
                raise ValueError("HC4-U4 case terminal accounting protocol mismatch")
    ...
    return compare_hc4u1_prescreen(
        ...
        protocol=f"HC4-U4-seed-{seed}-fixed-attempt-prescreen-v2",
    )
...
def main() -> None:
    ...
    result = compare_hc4u4_prescreen(
    ...
    print(f"hc4u4_decision={result['decision']}")
