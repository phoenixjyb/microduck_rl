"""F1-N: one predeclared neck-weight contrast, fixed selection and old gates."""
...
PROTOCOL = "f1n-neck-rate-paired-s499-v1"
OUTPUT = base.OUTPUT.parent / PROTOCOL
ARMS, WEIGHTS = ("control","neck"), dict(control=-.1,neck=-.2)
...
def prepare_config(mode, *, arm):
    require(arm in ARMS and mode in MODES,"predeclared neck arm/mode")
...
def paired_decision(parent,control,neck):
    return base.lateral.paired_decision(parent,control,neck,protocol=PROTOCOL,seeds=SEEDS)
...
def child(kind,arm,seed,source):
    ...
    if kind in MODES:
        ...
        write_new(p/"launch.json",dict(source=source,protocol=PROTOCOL,arm=arm,mode=kind,
            ...
            neck_weight=WEIGHTS[arm],motor_weight=-4.,lateral_weight=-.5,seed=expected_seed))
...
def run_child(kind,arm,source,end,*,seed=SEEDS[0]):
    ...
    with (OUTPUT/f"{name}.log").open("x") as log:
        result=subprocess.run([sys.executable,"-m","mjlab_microduck.foundation_neck_experiment",
...
def campaign(source):
    ...
    require(now<base.LAST_START and now+dt.timedelta(seconds=CAMPAIGN_SECONDS+90)<base.DEADLINE,"full F1-N budget before07:00")
    ...
    decision=dict(protocol=PROTOCOL,source=source,decision="runtime-failure-stop",reports=rows,pairs=pairs,
        pilot_slot=2,policy_acceptance=False,physical_motion_authorized=False,obstacles_admitted=False,
    ...
    try:
        write_new(OUTPUT/"launch.json",dict(protocol=PROTOCOL,source=source,retained=retained,runtime=runtime,
            idle=wait_idle(),neck_weights=WEIGHTS,pilot_slot=2,training_seed=499,smoke_seed=491,
        ...
        for seed in SEEDS:
            ...
            for mode in MODES:
                ...
                require(sha256(OUTPUT/f"control-{mode}/initial.pt")==sha256(OUTPUT/f"neck-{mode}/initial.pt"),
                        "identical paired restored learned state")
                a=json.loads((OUTPUT/f"control-{mode}/fingerprint.json").read_text())
                b=json.loads((OUTPUT/f"neck-{mode}/fingerprint.json").read_text())
            ...
            for seed in SEEDS:
                ...
                neck=evaluate("neck",seed)
                pair=paired_decision(parents[seed],control,neck);pairs.append(pair)
                if pair["failures"]:
...
def main():
    ...
    if args.child:
        ...
        require(launch["source"]==args.source and not (OUTPUT/"decision.json").exists(),"open exact F1-N campaign")
