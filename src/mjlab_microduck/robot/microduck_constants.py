...
def get_obstacle_spec() -> mujoco.MjSpec:
    return mujoco.MjSpec.from_file(str(MICRODUCK_OBSTACLE_XML))
...
# Free-moving but heavy obstacle box. Each episode places it relative to the
# robot with reset_obstacle_ahead; this initial pose only covers pre-first-reset.
MICRODUCK_OBSTACLE_CFG = EntityCfg(
    spec_fn=get_obstacle_spec,
    init_state=EntityCfg.InitialStateCfg(pos=(0.8, 0.0, 0.05)),
)
